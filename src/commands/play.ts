...
export async function cmd(message: Message, args: string[]){
	...
	message.reply(':musical_note: Playing ' + MusicPlayer.nowPlaying(message).title + ` (${MusicPlayer.nowPlaying(message).duration}) ` + ' [URL: ' + MusicPlayer.nowPlaying(message).url + ']');
}